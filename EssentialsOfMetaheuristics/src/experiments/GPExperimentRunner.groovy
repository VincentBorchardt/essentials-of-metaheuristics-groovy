...
class GPExperimentRunner {
	...
	static variableChance = 0.5
	static functionChance = 0.5
	static mutationRate = 0.3
	static crossoverChance = 0.3
	//static minX = 0
...
//		def functionList = [new Addition(), new Multiplication()]
		...
		def functionList = [new BoolIf(), new BoolOr(), new BoolAnd(), new BoolNot()]
		
		//def samplePoints = (0 .. defaultNumComparisonPoints).collect{i -> ["x": minX + (i * maxX / defaultNumComparisonPoints)]}//[["x": 0], ["x": 1]]
		
        def searchers = [
			new GeneticAlgorithm(crossover: new GPOnePointCrossover(defaultChance: crossoverChance).crossover, maxDepth: defaultMaxTreeSize)
			//new HillClimber()
