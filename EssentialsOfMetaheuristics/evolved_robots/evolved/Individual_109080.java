...
public class Individual_109080 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (29.00259494781494)*en.energy;
				eval += (62.67533302307129)*myEnergy;
				eval += (50.42511820793152)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (95.34670114517212)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
