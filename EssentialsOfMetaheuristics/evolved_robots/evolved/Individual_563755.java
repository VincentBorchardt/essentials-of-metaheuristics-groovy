...
public class Individual_563755 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (80.89651465415955)*en.energy;
				eval += (81.77320957183838)*myEnergy;
				eval += (94.1784918308258)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (73.74851703643799)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
