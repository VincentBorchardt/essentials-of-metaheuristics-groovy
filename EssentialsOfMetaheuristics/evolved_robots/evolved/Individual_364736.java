...
public class Individual_364736 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (7.188504934310913)*en.energy;
				eval += (81.17382526397705)*myEnergy;
				eval += (18.633466958999634)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (52.82542705535889)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
