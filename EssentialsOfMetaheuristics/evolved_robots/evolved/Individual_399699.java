...
public class Individual_399699 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (80.0094485282898)*en.energy;
				eval += (37.39932179450989)*myEnergy;
				eval += (36.6532564163208)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (16.899609565734863)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
