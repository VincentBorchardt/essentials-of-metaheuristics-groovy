...
public class Individual_998707 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (53.78599166870117)*en.energy;
				eval += (15.1236891746521)*myEnergy;
				eval += (1.4312684535980225)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (1.445448398590088)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
