...
public class Individual_574667 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (62.876975536346436)*en.energy;
				eval += (55.54053783416748)*myEnergy;
				eval += (66.35333299636841)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (6.350588798522949)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
