...
public class Individual_203388 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (81.5238893032074)*en.energy;
				eval += (55.86754083633423)*myEnergy;
				eval += (77.29656100273132)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (30.605977773666382)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
