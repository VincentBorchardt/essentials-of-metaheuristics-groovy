...
public class Individual_896118 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (26.447969675064087)*en.energy;
				eval += (91.44858121871948)*myEnergy;
				eval += (52.71952152252197)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (91.84467196464539)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
