...
public class Individual_579215 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (42.078787088394165)*en.energy;
				eval += (34.99276041984558)*myEnergy;
				eval += (75.30946135520935)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (11.668217182159424)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
