...
public class Individual_983722 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (12.118452787399292)*en.energy;
				eval += (62.32143044471741)*myEnergy;
				eval += (63.55879306793213)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (55.41844964027405)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
