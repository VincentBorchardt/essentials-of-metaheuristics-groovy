...
public class Individual_660268 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (22.41223454475403)*en.energy;
				eval += (94.75511312484741)*myEnergy;
				eval += (90.66264033317566)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (21.76269292831421)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
