...
public class Individual_28444 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (22.713810205459595)*en.energy;
				eval += (94.62517499923706)*myEnergy;
				eval += (40.23715853691101)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (16.020750999450684)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
