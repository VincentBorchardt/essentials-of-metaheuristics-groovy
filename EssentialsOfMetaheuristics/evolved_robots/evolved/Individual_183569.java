...
public class Individual_183569 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (98.3292281627655)*en.energy;
				eval += (5.836808681488037)*myEnergy;
				eval += (85.93316674232483)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (72.09498882293701)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
