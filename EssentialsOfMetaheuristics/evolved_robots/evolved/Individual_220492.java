...
public class Individual_220492 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (11.347591876983643)*en.energy;
				eval += (27.00129747390747)*myEnergy;
				eval += (89.45804834365845)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (33.1403911113739)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
