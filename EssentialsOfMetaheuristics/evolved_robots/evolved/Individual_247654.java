...
public class Individual_247654 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (9.161043167114258)*en.energy;
				eval += (72.09664583206177)*myEnergy;
				eval += (66.37824773788452)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (70.97581028938293)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
