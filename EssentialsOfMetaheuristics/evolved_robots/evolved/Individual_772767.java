...
public class Individual_772767 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (98.67335557937622)*en.energy;
				eval += (72.41811752319336)*myEnergy;
				eval += (46.90374732017517)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (28.602659702301025)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
