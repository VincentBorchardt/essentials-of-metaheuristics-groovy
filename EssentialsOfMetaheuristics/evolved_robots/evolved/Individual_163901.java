...
public class Individual_163901 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (41.59097075462341)*en.energy;
				eval += (26.01761221885681)*myEnergy;
				eval += (97.29565382003784)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (97.32200503349304)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
