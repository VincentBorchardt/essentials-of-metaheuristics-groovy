...
public class Individual_324315 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (53.52950096130371)*en.energy;
				eval += (35.211217403411865)*myEnergy;
				eval += (34.578871726989746)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (68.28927397727966)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
