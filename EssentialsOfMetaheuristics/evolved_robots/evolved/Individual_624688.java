...
public class Individual_624688 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (76.58648490905762)*en.energy;
				eval += (31.696414947509766)*myEnergy;
				eval += (29.372012615203857)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (2.297842502593994)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
