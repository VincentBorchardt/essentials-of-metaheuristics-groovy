...
public class Individual_710915 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (80.42166829109192)*en.energy;
				eval += (80.48717975616455)*myEnergy;
				eval += (17.02439785003662)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (16.03935956954956)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
