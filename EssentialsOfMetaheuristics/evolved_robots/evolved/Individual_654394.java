...
public class Individual_654394 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (37.11967468261719)*en.energy;
				eval += (87.05375790596008)*myEnergy;
				eval += (63.59759569168091)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (72.11809754371643)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
