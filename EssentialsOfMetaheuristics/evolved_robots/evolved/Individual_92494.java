...
public class Individual_92494 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (79.39620018005371)*en.energy;
				eval += (72.77852296829224)*myEnergy;
				eval += (19.961100816726685)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (66.83498024940491)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
