...
public class Individual_196666 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (39.96431827545166)*en.energy;
				eval += (77.19705104827881)*myEnergy;
				eval += (18.77831220626831)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (34.79452729225159)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
