...
public class Individual_325211 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (2.6086628437042236)*en.energy;
				eval += (99.45370554924011)*myEnergy;
				eval += (63.99948596954346)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (36.07189655303955)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
