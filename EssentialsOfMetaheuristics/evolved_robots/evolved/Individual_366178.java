...
public class Individual_366178 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (91.4933443069458)*en.energy;
				eval += (60.69849133491516)*myEnergy;
				eval += (51.87309980392456)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (59.89185571670532)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
