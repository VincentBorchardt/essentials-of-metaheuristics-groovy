...
public class Individual_392472 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (2.8551816940307617)*en.energy;
				eval += (19.46544647216797)*myEnergy;
				eval += (42.880088090896606)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (30.247408151626587)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
