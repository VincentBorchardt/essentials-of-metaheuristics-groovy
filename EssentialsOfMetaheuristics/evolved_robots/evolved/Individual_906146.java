...
public class Individual_906146 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (30.925780534744263)*en.energy;
				eval += (52.94071435928345)*myEnergy;
				eval += (23.28534722328186)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (55.5081844329834)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
