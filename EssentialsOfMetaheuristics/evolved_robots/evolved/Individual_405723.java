...
public class Individual_405723 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (51.959794759750366)*en.energy;
				eval += (2.7075886726379395)*myEnergy;
				eval += (82.17694163322449)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (86.22227907180786)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
