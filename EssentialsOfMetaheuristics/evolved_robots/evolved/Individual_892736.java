...
public class Individual_892736 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (73.25519323348999)*en.energy;
				eval += (33.858996629714966)*myEnergy;
				eval += (19.48000192642212)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (92.39749312400818)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
