...
public class Individual_635326 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (35.95595359802246)*en.energy;
				eval += (19.26923394203186)*myEnergy;
				eval += (50.98558068275452)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (71.71711921691895)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
