...
public class Individual_242536 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (89.79350328445435)*en.energy;
				eval += (73.03879857063293)*myEnergy;
				eval += (72.1475601196289)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (47.99773097038269)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
