...
public class Individual_735102 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (78.22979092597961)*en.energy;
				eval += (73.76521825790405)*myEnergy;
				eval += (47.32593297958374)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (19.578975439071655)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
